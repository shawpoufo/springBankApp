...
public class BankAccountServiceImpl implements BankAccountService {
    ...
    public CustomerDTO saveCustomer(CustomerDTO customerDTO) throws CustomerNotFoundException {
        ...
        saveCurrentBankAccount(0, 2.0, savedCustomer.getId());
        saveSavingBankAccount(0, 2.0, savedCustomer.getId());
    ...
    @Override
    public List<BankAccountDTO> bankAccountListByCustomerId(Long customerId) {
        List<BankAccount> bankAccounts = customerRepository.findById(customerId).get().getBankAccounts();
        List<BankAccountDTO> bankAccountDTOS = bankAccounts.stream().map(bankAccount -> {
            if (bankAccount instanceof SavingAccount) {
                SavingAccount savingAccount = (SavingAccount) bankAccount;
                return dtoMapper.fromSavingBankAccount(savingAccount);
